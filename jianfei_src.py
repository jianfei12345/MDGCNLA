...
def vis_result_ablation(result_ablation):
    # ROC 曲线
    fig_roc = go.Figure()
    fig_roc.add_shape(type='line', line=dict(dash='dash'), x0=0, x1=1, y0=0, y1=1)
    for keys in result_ablation.keys():
        fig_roc.add_trace(go.Scatter(x=result_ablation[keys]['roc'][0].cpu().numpy(),
                                     y=result_ablation[keys]['roc'][1].cpu().numpy(),
                                     name= keys+': AUROC =' + str(
                                         round(result_ablation[keys]['auroc'].item(), 3)), mode='lines'))
    fig_roc.update_layout(legend_x=0.85, legend_y=0, title='ROC curves', xaxis_title='FPR', yaxis_title='TPR')
    #fig_roc.show(renderer='browser')
    # PRC曲线
    fig_prc = go.Figure()
    fig_prc.add_shape(type='line', line=dict(dash='dash'), x0=1, x1=0, y0=0, y1=1)
    for keys in result_ablation.keys():
        fig_prc.add_trace(go.Scatter(x=result_ablation[keys]['pr'][1].cpu().numpy(),
                                     y=result_ablation[keys]['pr'][0].cpu().numpy(),
                                     name=keys + ': AUPRC =' + str(
                                         round(result_ablation[keys]['auprc'].item(), 3)), mode='lines'))
    fig_prc.update_layout(legend_x=0.15, legend_y=0, title='PRC curves', xaxis_title='Recall', yaxis_title='Precision')
    #fig_prc.show(renderer='browser')
    # subplot
    fig = make_subplots(rows=1, cols=2, subplot_titles=('ROC curves', 'PRC curves'))
    ...
    fig.update_yaxes(title_text='Precision', row=1, col=2)
    for keys in result_ablation.keys():
        fig.add_trace(go.Scatter(x=result_ablation[keys]['roc'][0].cpu().numpy(),
                                 y=result_ablation[keys]['roc'][1].cpu().numpy(),
                                 name=keys + ': AUROC =' + str(
                                     round(result_ablation[keys]['auroc'].item(), 3)), mode='lines', legendgroup=1), row=1,
                      col=1)
        fig.add_trace(go.Scatter(x=result_ablation[keys]['pr'][1].cpu().numpy(),
                                 y=result_ablation[keys]['pr'][0].cpu().numpy(),
                                 name=keys + ': AUPRC =' + str(
                                     round(result_ablation[keys]['auprc'].item(), 3)), mode='lines', legendgroup=2), row=1,
                      col=2)
    #fig.show(renderer='browser')
    return fig
...
#model construction: link prediction between A and B
class AB_GCNGAT(nn.Module):
    def __init__(self, Nlayer1, Nlayer2, Nlayer3, Dlayer1, Dlayer2, Dlayer3):
        super(AB_GCNGAT, self).__init__()
        self.Nconv1 = GCNConv(Nlayer1, Nlayer2)
        self.Nconv2 = GCNConv(Nlayer2, Nlayer3)
        self.NGAT1 = GATConv(Nlayer1, Nlayer3)

        self.Dconv1 = GCNConv(Dlayer1, Dlayer2)
        self.Dconv2 = GCNConv(Dlayer2, Dlayer3)
        self.DGAT1 = GATConv(Dlayer1, Dlayer3)


    def encoder_N_GCN(self, ncRNA_Data):
        nc_x, nc_edge_index, nc_edge_weight = ncRNA_Data.x, ncRNA_Data.edge_index, ncRNA_Data.edge_weight
        nc_x = self.Nconv1(nc_x, nc_edge_index, nc_edge_weight)
        nc_x = torch.relu(nc_x)
        nc_x = self.Nconv2(nc_x, nc_edge_index, nc_edge_weight)
    ...
    def encoder_N_GAT(self, ncRNA_Data):
        nc_x, nc_edge_index = ncRNA_Data.x, ncRNA_Data.edge_index
        nc_x = self.NGAT1(nc_x, nc_edge_index)
        return nc_x


    def encoder_D_GCN(self, dis_Data):
        dis_x, dis_edge_index, dis_edge_weight = dis_Data.x, dis_Data.edge_index, dis_Data.edge_weight
        dis_x = self.Dconv1(dis_x, dis_edge_index, dis_edge_weight)
        dis_x = torch.relu(dis_x)
        dis_x = self.Dconv2(dis_x, dis_edge_index, dis_edge_weight)
    ...
    def encoder_D_GAT(self, dis_Data):
        dis_x, dis_edge_index = dis_Data.x, dis_Data.edge_index
        dis_x = self.DGAT1(dis_x, dis_edge_index)
        return dis_x

    def decoder(self, nc_x, dis_x):
        return torch.sigmoid(torch.mm(nc_x, dis_x.t()))

    def forward(self, ncRNA_Data, dis_Data, encoder_name=['BOTH','BOTH']):
        encode_N_GCN = self.encoder_N_GCN(ncRNA_Data)
        encode_N_GAT = self.encoder_N_GAT(ncRNA_Data)
        if encoder_name[0] == 'GCN':
            encode_N = encode_N_GCN
        elif encoder_name[0] == 'GAT':
            encode_N = encode_N_GAT
        else:
            encode_N = torch.cat((encode_N_GCN, encode_N_GAT),1)

        encode_D_GCN = self.encoder_D_GCN(dis_Data)
        encode_D_GAT = self.encoder_D_GAT(dis_Data)
        if encoder_name[1] == 'GCN':
            encode_D = encode_D_GCN
        elif encoder_name[1] == 'GAT':
            encode_D = encode_D_GAT
        else:
            encode_D = torch.cat((encode_D_GCN, encode_D_GAT),1)
...
# cross validation in a splited dataset, return a total result, without model evaluation for each fold which may cause error due to the absense of positive samples, mainly for  loocv
def model_cv_nokfold_ablation_model(splited_data, model_name, label, epoch, input1, input2, label_mask, Nlayer3, Dlayer3, encoder_name):
    pred_all = torch.zeros_like(label).to(device)
    Nlayer1 = input1.x.size(0)
    Nlayer2 = round(((input1.x.size(0) * Nlayer3) ** 0.5) / 3)
    ...
    for i in range(len(splited_data)):
        ...
        for j in range(epoch):
            outputs = model(input1, input2, encoder_name)
            #loss = criterion(outputs[splited_data['Fold' + str(i)]['train_mask']],label[splited_data['Fold' + str(i)]['train_mask']].float())
            train_mask_epoch = ((negSampling_balanced_predefined_size(label, splited_data['Fold' + str(i)]['train_mask'].sum().item()).cpu().int() - (label.cpu() & splited_data['Fold' + str(i)]['test_mask'])) == 1)
            loss = criterion(outputs[train_mask_epoch], label[train_mask_epoch].float())
            optimizer.zero_grad()
...
def run_model_negSamp_epoch(Dataset, model_name,parameters_topk, parameters_embedding, encoder_name=dict(GCNGCN=['GCN','GCN'], GCNGAT=['GCN','GAT'],GATGAT=['GAT','GAT'], GATGCN=['GAT', 'GCN'], both=['both','both']),topk=15, k_fold=5, epoch=200, Nlayer3=10, Dlayer3=10, pred_k_times=100, localcv_topk=10, localcv_kfold=10, robustness_p_zero = [0.2,0.5,0.8,0.95], robustness_p_noise = [0.02,0.04,0.06,0.08,0.1], casestudy_nrow=5, casestudy_ncol=5, casestudy_topk=5):
    ...
    output_localcv = local_cv_row_col_topk(Dataset1_circ, Dataset1_dis, Dataset,model_name, localcv_topk, localcv_kfold, epoch)
    time_end3 = time.time()

    #需要负抽样，不需要加权交叉熵。为什么和交叉验证结果相差甚远，因为下面的函数没有每次epoch负抽样，需要每次epoch负抽样。计算加权交叉熵权重的时候需要固定数据集，所以加权交叉熵和负抽样不能同时存在
    output_parameters = parameters_experiment(label, label_mask, Dataset, model_name, parameters_topk, parameters_embedding, epoch)
    time_end4 = time.time()
    ...
    output_robustness = robustness_experiment(Dataset, model_name, robustness_p_zero, robustness_p_noise,epoch)
    time_end5 = time.time()

    result_ablation = experiment_ablation_model(Dataset, model_name, encoder_name, epoch=epoch)
    time_end6 = time.time()
    ...
    time_experiment = pd.DataFrame(columns=['Time consuming(s)','kfold','k_times','topk','embedding','robustness_p','epoch'], index=['CV','Prediction','local_cv','parameters','robustness','ablation', 'total'])
    ...
    time_experiment.loc['ablation', 'Time consuming(s)'] = round(time_end6 - time_end5, 2)
    time_experiment.loc['total', 'Time consuming(s)'] = round(time_end6 - time_start, 2)
    ...
    time_experiment.loc['robustness', 'robustness_p'] = [str(robustness_p_zero),str(robustness_p_noise)]
    print('model:'+str(model_name),'\n', 'Cross validation:','Kfold:'+str(k_fold), 'topk:'+str(topk), 'epoch:'+str(epoch), 'input1_embedding:'+str(Nlayer3), 'input2_embedding:'+str(Dlayer3),'\n','Prediction:','topk:'+str(topk), 'epoch:'+str(epoch), 'input1_embedding:'+str(Nlayer3), 'input2_embedding:'+str(Dlayer3),'k_times:'+str(pred_k_times), '\n', 'local_cv:','kfold:5','epoch:200','\n','parameters experiment:','topk:'+str(parameters_topk),'embedding:'+str(parameters_embedding),'\n','robustness experiment:','p'+str(robustness_p_noise),'\n', start_time, '\t', str(k_fold)+'fold-CV:', round(time_end1 - time_start, 2), 's', '\t', str(pred_k_times)+'times-pred:', round(time_end2 - time_end1, 2), 's', '\t','local_cv:', round(time_end3 - time_end2, 2), 's', '\t','parameters_experiment:', round(time_end4 - time_end3, 2), 's', '\t','robustness_experiment:', round(time_end5 - time_end4, 2), 's', '\t','total_time:', round(time_end6 - time_start, 2), 's', '\t', end_time)

    return dict( result_cv = result_cv, result_pred = pred, result_localcv = output_localcv, output_parameters = output_parameters, result_ablation = result_ablation, output_robustness = output_robustness, result_case_study=output_case_study, result_noval_prediction = output_noval_prediction, result_time = time_experiment)





def InterAB_Run(Dataset, model, encoder_name = dict(GCN_GCN=['GCN','GCN'], GCN_GAT=['GCN','GAT'],GAT_GAT=['GAT','GAT'], GAT_GCN=['GAT', 'GCN'], both_both=['both','both']),epoch=200,Nlayer3=10, Dlayer3=10, pred_k_times=100, localcv_topk=10,localcv_kfold=20,parameters_topk=[5, 10, 15, 20, 25, 30],parameters_embedding=[2, 6, 10, 15, 20],robustness_p_zero=[0.2, 0.5, 0.8, 0.9, 0.95], robustness_p_noise=[0.02,0.04,0.06,0.08,0.1,0.3,0.5], casestudy_nrow=5, casestudy_ncol=5, casestudy_topk=5):
    #Dataset: 字典，多个数据集
    #model: AB链路预测模型
    ...
    for keys in Dataset.keys():
        output[keys] = run_model_negSamp_epoch(Dataset[keys], model_name=model,
                                                           ...
                                                           parameters_embedding=parameters_embedding, encoder_name=encoder_name, k_fold=[5,10],
                                                           ...
                                                           robustness_p_zero=robustness_p_zero, robustness_p_noise=robustness_p_noise, casestudy_nrow=casestudy_nrow, casestudy_ncol=casestudy_ncol, casestudy_topk=casestudy_topk)
    vis_all(output)
    if len(Dataset)>1:
        venn_plot(Dataset)
...
#ablation experiment for model conposition
def experiment_ablation_model(Dataset, model_name, encoder_name, epoch, topk=15, k_fold=5, Nlayer3=10, Dlayer3=10):
    #encoder_name: a dictionary, each value should be a list with two elements. for example: ['GCN','GCN'],['GCN', 'GAT],
    result_ablation = {}
    Dataset1_circ = sim_to_torchgeometric_Data(Dataset['input1'], topk).to(device)
    Dataset1_dis = sim_to_torchgeometric_Data(Dataset['input2'], topk).to(device)
    ...
    for keys in encoder_name.keys():
        result_ablation[keys] = model_cv_nokfold_ablation_model(splited_data, model_name, label, epoch, Dataset1_circ,Dataset1_dis, label_mask, Nlayer3, Dlayer3, encoder_name[keys])
...
def local_cv_col_topk(Dataset1_circ, Dataset1_dis, Dataset, model_name, topk, k_fold, epoch):
    # top5最大值索引
    ...
    output_local_cv = {}
    for i in range(topk):
        output_local_cv[str(keys[i])] = local_LOOCV_col(Dataset1_circ, Dataset1_dis, index_topk[i], k_fold, model_name, Dataset, epoch, 10, 10)
...
def local_cv_row_topk(Dataset1_circ, Dataset1_dis, Dataset, model_name, topk, k_fold, epoch):
    # top5最大值索引
    ...
    output_local_cv = {}
    for i in range(topk):
        output_local_cv[str(keys[i])] = local_LOOCV_row(Dataset1_circ, Dataset1_dis, index_topk[i], k_fold, model_name, Dataset, epoch, 10, 10)
...
def local_cv_row_col_topk(Dataset1_circ, Dataset1_dis, Dataset,model_name, topk, kfold, epoch):
    output_row = local_cv_row_topk(Dataset1_circ, Dataset1_dis, Dataset,model_name, topk, kfold, epoch)
    output_col = local_cv_col_topk(Dataset1_circ, Dataset1_dis, Dataset,model_name, topk, kfold, epoch)
...
def parameters_effect_topk(label, label_mask, Dataset, model_name,parameters_topk, epoch):
    output_parameters_topk = {}
    for i in parameters_topk:
        output_parameters_topk[str(i)] = parameters_effect(label, label_mask, 5, model_name, Dataset, epoch, topk=i, Nlayer3=10,Dlayer3=10)

    output = local_cv_summary(output_parameters_topk, parameters_topk)
    return output

def parameters_effect_embedding_size(label, label_mask, Dataset, model_name,parameters_embedding, epoch):
    output_parameters_embedding = {}
    for i in parameters_embedding:
        output_parameters_embedding[str(i)] = parameters_effect(label, label_mask, 5,model_name,Dataset,epoch,15,Nlayer3=i,Dlayer3=i)

    output = local_cv_summary(output_parameters_embedding, parameters_embedding)
    return output

def parameters_experiment(label, label_mask, Dataset, model_name, parameters_topk, parameters_embedding, epoch):
    output = {}
    output['topk'] = parameters_effect_topk(label, label_mask, Dataset, model_name, parameters_topk, epoch)
    output['embedding'] = parameters_effect_embedding_size(label, label_mask, Dataset, model_name, parameters_embedding, epoch)

    return output



def robustness_experiment_zero(Dataset, model_name, p, epoch):
    output_row = {}
    output_col = {}
    output_both = {}
    output = {}
    ...
    for i in p:
        ...
        output_row[str(i)] = experiment_cv(Dataset_temp, model_name, epoch=epoch)
    #col perturbation
    for i in p:
        Dataset_temp = copy.deepcopy(Dataset)
        Dataset_temp['input2'] = random_perturbation_zero_matrix(Dataset_temp['input2'], i)
        output_col[str(i)] = experiment_cv(Dataset_temp, model_name, epoch=epoch)
    #both row and col perturbation
    for i in p:
        Dataset_temp = copy.deepcopy(Dataset)
        Dataset_temp['input1'] = random_perturbation_zero_matrix(Dataset_temp['input1'], i)
        Dataset_temp['input2'] = random_perturbation_zero_matrix(Dataset_temp['input2'], i)
        output_both[str(i)] = experiment_cv(Dataset_temp, model_name, epoch=epoch)
...
def robustness_experiment_noise(Dataset, model_name, p, epoch):
    output_row = {}
    output_col = {}
    output_both = {}
    output = {}
...
def robustness_experiment(Dataset, model_name, p_zero, p_noise, epoch):
    output = {}
    output_zero = robustness_experiment_zero(Dataset, model_name, p_zero, epoch)
    output_noise = robustness_experiment_noise(Dataset, model_name, p_noise, epoch)
...
def vis_all(outputs):
    ...
    for key in outputs.keys():
        ...
        os.makedirs(os.path.join(key, 'ablation'))
        fig = vis_result_ablation(outputs[key]['result_ablation'])
        fig.write_image(os.path.join(key, 'ablation', 'model_ablation.pdf'), width=780, height=380)
    #返回到远工作目录
...
def latex_output(output):
    print(output['result_localcv']['row'].to_latex(caption='local cv row'))
    print(output['result_localcv']['col'].to_latex(caption='local cv col'))
    print(output['output_parameters']['topk'].to_latex(caption='parameter topk'))
    print(output['output_parameters']['embedding'].to_latex(caption='parameter embedding'))
    print(output['output_robustness']['zero']['both'].to_latex(caption='robustness zero both'))
    print(output['output_robustness']['noise']['both'].to_latex(caption='robustness noise both'))
    print(output['result_case_study']['row'].to_latex(caption='case study row'))
    print(output['result_case_study']['col'].to_latex(caption='case study col'))
